...
/* OPTIMIZATION: Add 'use strict' to all funtions to make the code more secure and to make it
  easier to JavaScript engines to perform optimizations.
*/
...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    /* OPTIMIZATION: Use getElementById (faster) instead of querySelector*/
    switch (size) {
      case "1":
  ...
  function changePizzaSizes(size) {
    /* OPTIMIZATION:
     Read the DOM to get randomPizzaCointainers only once outside the loop.
     Use getElementsByClassName (faster) instead of querySelectorAll
     Since all are the same size, no need to read each pizzaContainer. Just get
     dx and newwidth from the first element (also outside the loop). Reading
     offsetWidth once outside the loop avoids forced reflow.
    */
  ...
  /* OPTIMIZATION:
    Call changeSliderLabel (DOM manipulation) after reading old pizza sizes to
    avoid unecessary layout recalculation since it is recalculated again when changing
    pizza sizes.
  */
...
/* OPTIMIZATION: get randomPizzas only once outside the loop; */
...
function updatePositions() {
  ...
  /* OPTIMIZATION:
    Read "body.scroll" outside the loop to avoid forced synchronous layout.
    Do posible calculations once (outside the loop)
  */
  var body_scrollY = document.body.scrollTop / 1250;
  var phases = [
    Math.sin((body_scrollY)),
    Math.sin((body_scrollY) + 1),
    Math.sin((body_scrollY) + 2),
    Math.sin((body_scrollY) + 3),
    Math.sin((body_scrollY) + 4)
  ];
  var phase;
  var items_length = items.length;
  for (var i = 0; i < items_length; i++) {
    /* OPTIMIZATION:
       Use transform property instead of left -> Uses CSS3 hardware acceleration */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  /* OPTIMIZATION:
    Query movingpizzas element once (outside loop).
    Use getElementById instead of querySelector (is faster)
    Calculate maximum number of pizzas in the screen to avoid creating 200
  */
  ...
  /* OPTIMIZATION:
    Declare the elem variable (var elem;) in the initialisation of the for-loop
    to prevent it from being created every time the loop is executed.
  */
  for (var i = 0, elem; i < numberPizzas; i++) {
    elem = document.createElement('img');
    elem.className = 'mover';
    elem.src = "images/pizza.png";
    elem.style.height = "100px";
    elem.style.width = "73.333px";
    /* OPTIMIZATION: Since I am using transform: tranlateX in upadatePositions,
       setting the initial left position is needed. */
